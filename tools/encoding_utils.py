...
This module provides a command line interface as the primary means of
invocation.

@var DEFAULT_ENCODING: default encoding to use
@type DEFAULT_ENCODING: L{str}
"""
...
def get_encoding_from_line(s):
    """
    Return the encoding specified in the given line or None if none was
    specified.   
    
    @param s: line to check
    @type s: L{str}
    @return: the encoding specified in this line or None
    @rtype: L{bool} or{ None}
    """
...
def set_all_encodings(p, encoding, recursive=False, ignore_nonpy=False, force=False):
    ...
    @param ignore_nonpy: skip files not ending with .py
    @type ignore_nonpy: L{bool}
    @param force: even set the encoding of a file if it already has an
    encoding
    @type force: L{bool}
    """
    for fn in os.listdir(p):
